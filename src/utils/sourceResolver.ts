...
export function buildSourceInfoMap(input: SourceInfoMapInput): SourceInfoMap {
  ...
  const providers: Array<{
    items: Array<{ apiKey?: string; prefix?: string; name?: string; authIndex?: string }>;
    type: string;
    label: string;
  }> = [
  ...
  providers.forEach(({ items, type, label }) => {
    items.forEach((item, index) => {
      registerProvider(
        {
          displayName: item.name?.trim() || item.prefix?.trim() || `${label} #${index + 1}`,
          type,
  ...
  (input.openaiCompatibility || []).forEach((provider, providerIndex) => {
    ...
    registerProvider(
      {
        displayName: provider.name?.trim() || provider.prefix?.trim() || `OpenAI #${providerIndex + 1}`,
        type: 'openai',
